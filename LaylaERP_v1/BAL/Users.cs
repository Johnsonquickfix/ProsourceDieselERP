    ...
    public class Users
    {
        ...
        public static DataTable GetRolesTypeTopBar()
        {
            ...
            try
            {
                ...
                string strquery = "select User_Type,User_Value,(select count(umc.user_id) from wp_usermeta umc where umc.meta_key = 'wp_capabilities' and umc.meta_value like CONCAT('%', User_Value, '%')) cnt from wp_user_classification as ur order BY ur.User_Type";
        ...
        public static DataTable GetDetailsUser(long ID)
        {
            ...
            try
            {
                ...
                string strquery = "select ID,user_login, user_status,case when user_status=0 then 'Active' else 'InActive' end as status,user_email,display_name,User_nicename,"
                                ...
                                + " where id = "+ ID + " group by u.id,user_login, user_status,user_status,user_email,display_name,User_nicename";
                DT = SQLHelper.ExecuteDataTable(strquery);
            }
            catch (Exception ex)
            { throw ex; }
