            ...
            try
            {
                ...
                else if (model.Status == 3)
                {
                    ...
                    strsql += "insert into erp_accounting_bookkeeping (entity,inv_num,doc_date,doc_type,doc_ref,PO_SO_ref,fk_doc,fk_docdet,thirdparty_code,subledger_account,subledger_label,inv_complete,label_complete,label_operation,debit,credit,invtotal,senstag,fk_user_author,date_creation,code_journal,journal_label,fk_bank)"
                               + " select 1,cpo.rowid,'" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','PO',ref_ext,ref,1,0,ref_supplier,'5010',name vname,'5010','Product Cost',Concat(name,'-',ref_supplier,'-Subledger account') lb_opration,'0.00',format(total_ttc,2),total_ttc,'C','1','" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','AC','Purchase journal','0' from commerce_purchase_order cpo inner join wp_vendor wpvn on wpvn.rowid = cpo.fk_supplier and VendorStatus=1 where cpo.rowid = " + model.Search + ""
                               + " Union all select 1,cpo.rowid,'" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','PO',ref_ext,ref,1,0,ref_supplier,'','','5950','Purchase Discount',Concat(name,'-',ref_supplier,'-Purchase Discount') lb_opration,'0.00',format(cpo.discount,2),cpo.discount,'C','1','" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','AC','Purchase journal','0' from commerce_purchase_order cpo inner join wp_vendor wpvn on wpvn.rowid = cpo.fk_supplier and VendorStatus=1 where cpo.discount > 0 and cpo.rowid = " + model.Search + ""
                               + " Union all select 1,cpo.rowid,'" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','PO',ref_ext,ref,1,0,ref_supplier,'','','5010','Product Cost',Concat(name,'-',ref_supplier,'-Product Cost') lb_opration,format(total_ht,2),'0.00',total_ht,'D','1','" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','AC','Purchase journal','0' from commerce_purchase_order cpo inner join wp_vendor wpvn on wpvn.rowid = cpo.fk_supplier and VendorStatus=1 where cpo.rowid = " + model.Search + ""
                               + " Union all select 1,cpo.rowid,'" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','PO',ref_ext,ref,1,0,ref_supplier,'','','1234','Tax',Concat(name,'-',ref_supplier,'-Tax') lb_opration,format(localtax1,2),'0.00',localtax1,'D','1','" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','AC','Purchase journal','0' from commerce_purchase_order cpo inner join wp_vendor wpvn on wpvn.rowid = cpo.fk_supplier and VendorStatus=1 where localtax1 > 0 and cpo.rowid = " + model.Search + ""
                               + " Union all select 1,cpo.rowid,'" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','PO',ref_ext,ref,1,0,ref_supplier,'','','5800','Cost of Good',Concat(name,'-',ref_supplier,'-Cost of Good') lb_opration,format(localtax2,2),'0.00',localtax2,'D','1','" + cDate.ToString("yyyy-MM-dd HH:mm:ss") + "','AC','Purchase journal','0' from commerce_purchase_order cpo inner join wp_vendor wpvn on wpvn.rowid = cpo.fk_supplier and VendorStatus=1 where localtax2 > 0 and cpo.rowid = " + model.Search + "";
                }
                else
