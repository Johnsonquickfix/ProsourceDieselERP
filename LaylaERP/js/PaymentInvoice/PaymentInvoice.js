...
function formatPartially(d) {
    ...
    let option = { strValue1: d.id }, wrHTML = '<table class="inventory-table table-blue table check-table table-bordered table-striped dataTable no-footer"><thead><tr><th style="width:20%; text-align:left;">Bill No</th><th style="width:35%; text-align:left;">Receive Date</th><th style="width:30%; text-align:left;">Paid Amount</th></tr></thead>';
    $.ajax({
        ...
        success: function (result) {
            ...
            $(result).each(function (index, row) {
                ...
                //wrHTML += '<td style="width:40%; text-align:right;">' + row.amount + '</td></tr > ';
...
function PurchaseOrderGrid(is_date) {
    ...
    $('#dtdata').DataTable({
        ...
        destroy: true, ajax: {
            url: '/PaymentInvoice/GetPOOrderList', type: 'GET', dataType: 'json', contentType: "application/json; charset=utf-8", data: obj,
            dataSrc: function (data) { return JSON.parse(data); }
        },
        lengthMenu: [[10, 20, 50, 100], [10, 20, 50, 100]],
        columns: [
