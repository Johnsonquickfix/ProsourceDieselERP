$(document).ready(function () {
    ...
    let urlperm = "/PurchaseProduct/ListProduct";
    ...
    $("#btnattributerefresh").click(function (e) {
        ...
        $.ajax({
            url: '/PurchaseProduct/Getlatestattributesbyid', type: 'post', contentType: "application/json; charset=utf-8", dataType: 'JSON',
    ...
    $("#addvariation").click(function (e) {
        ...
        if (ID != "") {
            $.ajax({
                type: 'POST',
                url: '/PurchaseProduct/savevariantproduct',
            ...
            $.each(_Attlist, function (key, value) {
                ...
                if (value && value.key && value.value) {
                    ...
                    varHTML += '<option value="' + cleanedKey + '" selected="">' + (cleanedKey ? cleanedKey : 'Any') + '</option>';
                    ...
                    for (let j = 0; j < _values.length; j++) {
                        ...
                        // Replace "No default" from the option display
                        optionDisplay = optionDisplay.replace("No default ", "Any ");
                    ...
                    // Set the selected attribute if the key is found
                    if (keyFound) {
                        varHTML = varHTML.replace('<option value="' + cleanedKey + '" selected="">' + (cleanedKey ? cleanedKey : 'Any') + '</option>', '<option value="' + cleanedKey + '" selected="">' + (cleanedKey ? cleanedKey : 'Any') + '</option>');
                    }
        ...
        if (_attxml != '') {
            ...
            if (less == 1)
                ...
                if (checkstr == true) {
                    $.ajax({
                        type: 'POST',
                        url: '/PurchaseProduct/Savevariations',
                        ...
                        success: function (data) {
                            if (data.status == true) {
                                ...
                                ActivityLog('save variation for product id(' + id + ')', '/PurchaseProduct/AddNewProduct/' + id + '');
                                //GetProductvariationID(id);
                            }
                            // EMPTY THE ARRAY.
                            // alert(response.d);
    ...
    $("#btnsaveAttributes").click(function (e) {
       ...
       // if (values != '') {
            ...
            if (checkstr == true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/PurchaseProduct/saveproductAttributes',
                    ...
                    success: function (data) {
                        if (data.status == true) {
                            if (data.url == "Manage") {
                                ...
                                //GetProductvariationID(id);
                                swal('Success!', data.message, 'success');
                                ActivityLog('save attribute for product id(' + id + ')', '/PurchaseProduct/AddNewProduct/' + id + '');
                            }
                            else {
...
function AddProduct() {
    ...
    if (productname == "") {
    ...
    else if (regularprice == "" && ProductTypeval != 11) {
        ...
        if (checkstr == true) {
            $.ajax({
                url: '/PurchaseProduct/CreateProduct/', dataType: 'json', type: 'Post',
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify(obj),
                dataType: "json",
                beforeSend: function () {
...
function GetDataByID(order_id) {
    ...
    $.ajax({
        ...
        url: '/PurchaseProduct/GetDataByID/' + ID,
        type: 'post',
        contentType: "application/json; charset=utf-8",
        dataType: 'JSON',
        data: JSON.stringify(obj),
        success: function (data) {
            ...
            $('#ddltaxstatus').val(data.tax_status).trigger('change');
            ...
            $('#ddlclasstax').val(data.tax_class).trigger('change');
...
function GetProductvariationID(ProductID) {
    ...
    $.ajax({
        url: '/PurchaseProduct/Getvariationdetailsbyid', type: 'post', contentType: "application/json; charset=utf-8", dataType: 'JSON',
        data: JSON.stringify(obj),
        success: function (data) {
            data = JSON.parse(data);
           // console.log(data);
                    ...
                    variationAttributes.forEach(function (attribute) {
                       ...
                       // console.log('displaynm', attribute.display_name);
                        ...
                        if (typeof attribute.option === 'string') {
                            ...
                            // Loop through options for the current attribute
                            attribute.option.forEach(function (option) {
                                //console.log('ss', option, option.term_id, option.name);
                                //sel_val = option.term_id;
                                // Create an option element for each option
                                // varHTML += '<option value="' + option.term_id + '">' + option.name + '</option>';
                                varHTML += '<option value="' + option.term_id + '" ' + (parseInt(sel_vals) === option.term_id ? 'selected' : '') + '>' + option.name + '</option>';
