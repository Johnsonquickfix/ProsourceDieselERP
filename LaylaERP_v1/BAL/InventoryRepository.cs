...
namespace LaylaERP.BAL
{
    public class InventoryRepository
    {
        ...
        public static DataTable ProductInventoryReport(string sMonth)
        {
            DataTable dt = new DataTable();
            ...
            try
            {
                ...
                string strSql = "SELECT p.id,p.post_type,p.post_title,s.meta_value sku,(case when p.post_parent = 0 then p.id else p.post_parent end) p_id,p.post_parent,"
                                + " (select coalesce(sum(case pwr_o.flag when 'R' then pwr_o.quantity when 'T' then pwr_o.quantity when 'I' then - pwr_o.quantity else 0 end),0) from vw_product_stock_register pwr_o where pwr_o.product_id = p.id and pwr_o.flag != 'O' " + strWhr2 + ") op_stock,"
                                ...
                                + " left outer join vw_product_stock_register pwr on pwr.product_id = p.id " + strWhr + ""
