...
function PurchaseOrderGrid() {
    ...
    $('#dtdata').DataTable({
        ...
        sAjaxSource: "/PaymentInvoice/GetPurchaseSOOrderList",
...
function formatPO(d) {
    let option = { strValue1: d.id }, wrHTML = '<table class="inventory-table1 table-blue table check-table table-bordered table-striped dataTable no-footer"><thead><tr><th style="width:20%; text-align:left; visibility:inherit; opacity:1;">Bill No</th><th style="width:35%; text-align:left; visibility:inherit; opacity:1;">Receive Date</th><th style="width:15%; text-align:left; visibility:inherit; opacity:1;">Transaction Id</th><th style="width:30%; text-align:left; visibility:inherit; opacity:1;">Paid Amount</th></tr></thead>';
    $.ajax({
        url: '/PaymentInvoice/GetPaymentDataListSO', type: 'post', dataType: 'json', contentType: "application/json; charset=utf-8", data: JSON.stringify(option),
        success: function (result) {
            ...
            $(result).each(function (index, row) {
                wrHTML += '<tr><td style="width:20%; text-align:left;"> ' + row.ref + ' </td><td style="width:35%; text-align:left;">' + row.date_creation + '</td><td style="width:15%; text-align:left;">' + row.transaction_id + '</td>';
...
function takepayment() {
  ...
  //  console.log(id);
    if (id == "") {
        ...
        ActivityLog('Go for payment process', '/PaymentInvoice/PaymentInvoiceSO?status=' + status + '&id=' + id + '');
        var name = $("#txtName").val();
        var tech = $("#ddlTechnolgy").val();
        /*var url = "Page2.htm?status=" + encodeURIComponent(name) + "&technology=" + encodeURIComponent(tech);*/
        var url = "/PaymentInvoice/PaymentInvoiceSO?status=" + status + "&id=" + id;
