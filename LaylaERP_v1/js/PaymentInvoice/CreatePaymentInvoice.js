                ...
                try {
                    ...
                    for (let i = 0; i < data['pod'].length; i++) {
                        ...
                        if (data['pod'][i].rowid > 0) {
                            itemHtml = '<tr id="tritemid_' + data['pod'][i].rowid + '" class="paid_item" data-pid="' + data['pod'][i].rowid + '" data-supplier="' + data['pod'][i].ref_supplier + '" data-billno="' + data['pod'][i].ref_ext + '" data-rowid="' + data['pod'][i].rowid + '">';
            ...
            success: function (result) {
                try {
                    ...
                    for (let i = 0; i < data['pod'].length; i++) {
                        let itemHtml = '';
                        if (data['pod'][i].rowid > 0) {
                            itemHtml = '<tr id="tritemid_' + data['pod'][i].rowid + '" class="paid_item" data-pid="' + data['pod'][i].rowid + '" data-supplier="' + data['pod'][i].ref_supplier  + '" data-billno="' + data['pod'][i].ref_PO + '" data-rowid="' + data['pod'][i].rowid + '">';
                            itemHtml += '<td>' + data['pod'][i].ref_PO + '</td>';
                            itemHtml += '<td>' + data['pod'][i].ref_PI + '</td>';
                            itemHtml += '<td class="text-left">' + data['pod'][i].date_creation + '</td>';
                            itemHtml += '<td class="text-left">' + data['pod'][i].date_livraison + '</td>';
                            itemHtml += '<td class="text-right ship-amount">$' + data['pod'][i].total_ttc.toFixed(2) + '</td>';
                            itemHtml += '<td class="text-right row-total">$' + data['pod'][i].recieved.toFixed(2) + '</td>';
...
function savePayment() {
    ...
    if (PaymentTypeid <= 0) { swal('Error', 'Please Select Payment Type', 'error').then(function () { swal.close(); $('#ddlPaymentType').focus(); }) }
        ...
        swal.queue([{
            ...
            preConfirm: function () {
                return new Promise(function (resolve) {
                    $.post('/PaymentInvoice/TakePaymentPO', option).done(function (result) {
                        result = JSON.parse(result);
                        if (result[0].Response == "Success") {
                            if (status == 'PO') {
                                swal('Success', 'Payment has been taken successfully!!', 'success').then((result) => { location.href = 'PaymentInvoiceList'; });
...
function createItemsList() {
    ...
    $('#line_items > tr').each(function (index, row) {
        ...
        if (payment > 0) {
            _list.push({ fk_payment: 0, fk_invoice: $(row).data('rowid'), fk_invoceso: $(row).data('billno'), amount: payment, type: status, thirdparty_code: $(row).data('supplier'), checkstatus: checkstatus, payee: payeeval});
