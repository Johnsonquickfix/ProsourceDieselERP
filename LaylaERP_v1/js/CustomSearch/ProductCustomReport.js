...
function getMasters() {
    ...
    $('#ddlCategory').append('<option value="">Please select account</option>');
    $.ajax({
        url: "/customsearch/filtermasters", type: "Get", data: { strValue1: 'PRODUCT' },
        ...
        success: function (data) {
            ...
            //Payment Type
            $.each(data['Table1'], function (i, row) { $("#ddlCategory").append('<option value="' + row.id + '">' + row.text + '</option>'); });
...
function dataGridLoad() {
    ...
    let table_oh = $('#dtordersearch').DataTable({
        ...
        sAjaxSource: "/customsearch/product-list",
...
function ExportList() {
    let _columns = [];
    let _order_status = $("#ddlStatus").val().map(d => `'${d}'`).join(','), _order_payment = $("#ddlCategory").val().map(d => `'${d}'`).join(',');
    let _display_field = [], _where_field = [];
    $("#ddlDisplayField :selected").each(function (e, r) {
        _display_field.push({ strType: $(r).data('tb_type'), strKey: $(r).text(), strValue: $(r).val() });
