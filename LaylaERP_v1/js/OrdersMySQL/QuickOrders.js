$(document).ready(function () {
    ...
    $(document).on("click", "#btnaddproduct", function (t) {
        AddproductModal(0, 0, false, false);
    });
...
// Assuming you have a button with the id "addButton"

$(document).on("click", "#addButton", function (t) {
    // Create a unique ID for each new set of dropdown and input
    var uniqueId = new Date().getTime();
    console.log(uniqueId)

    var dynamicHtml = '<tr>';
    dynamicHtml += '<td style="width: 150px;"><select class="select2 ddlProductmodel" id="ddlProductmodel' + uniqueId + '" placeholder="Select Products" style="width: 80%;"></select></td>';
    // Add input text
    dynamicHtml += '<td style="width: 100px;"><input type="text" class="form-control" id="textInput' + uniqueId + '" placeholder="quantity"></td>';
    dynamicHtml += '</tr>';
    // Append the dynamic set to the modal body 
    $('#tbladdproduct').append(dynamicHtml);
    let productData = {};
    //$(".ddlProductmodel").select2({
    //    allowClear: true,
    //    minimumInputLength: 3,
    //    placeholder: "Search Product",
    //    dropdownParent: $("#billModal"), // Ensure dropdown appears within modal
    //    ajax: {
    //        url: '/Quickorder/GetProductList',
    //        type: "POST",
    //        contentType: "application/json; charset=utf-8",
    //        dataType: 'json',
    //        delay: 250,
    //        data: function (params) {
    //            var obj = { strValue1: params.term, strValue2: $("#ddlproducttype").val() };
    ...
    $("#ddlProductmodel" + uniqueId).select2({
        allowClear: true,
        minimumInputLength: 3,
        placeholder: "Search Product",
        dropdownParent: $("#billModal"), // Ensure dropdown appears within modal
        ajax: {
            url: '/Quickorder/GetvariationProductList',
            type: "POST",
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
            delay: 250,
    ...
    //    // Get the selected value and append it to the productData object
    //    var selectedValue = e.params.data.id;
    //    productData[selectedValue] = {
    //        text: e.params.data.text,
    //        parent_id: e.params.data.post_parent
    //    };

    //    console.log(productData);
    //});

    // After the dropdown is initialized, set the selected value
    // Replace 'yourSelectedValue' with the actual ID you want to set
    //$("#ddlProductmodel" + uniqueId).val('yourSelectedValue').trigger('change');
});
$(document).on("click", "#btnaddsession", function () {
    let cartItems = [];
    // Iterate over each row 
    $('#tbladdproduct tr').each(function () {
        // Find the select and input elements within the current row
        //let $row = $(this);
        //let parentId = $row.find('.parent-id').data('parentid');
        //let productid = $row.find('.parent-id').data('productid');
        //let itemQuantity = parseFloat($row.find('input[name="txt_itemqty"]').val()); // Get value by name 
        var ddlProductmodel = $(this).find('.ddlProductmodel');
        var textInput = $(this).find('input[type="text"]');
        // Get the selected value from the dropdown
        var selectedValue = ddlProductmodel.val();
        ...
        if (typeof selectedValue === 'undefined') {
            ...
            let productid = parseInt(parts[0]) || 0;
            let parentId = parseInt(parts[1]) || 0;
            // Get the input text value
            let itemQuantity = textInput.val();
            // Log or use the values as needed
            console.log(parts, productid, parentId, itemQuantity);
            if (parentId > 0) {
                //variation
                cartItems.push({
                    id: parentId,
...
function AddproductModal(paymode, id, is_mail, is_back) {
    var modalHtml = '';
    modalHtml += '<div class="modal-dialog modal-lg" style="height: 80vh; z-index: 1050;">'; // Increase the z-index
    modalHtml += '<div class="modal-content">';
    modalHtml += '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>';
    modalHtml += '<h4 class="modal-title" id="myModaladdproduct">Add Product</h4>';
    modalHtml += '<div class="modal-body no-padding"></div>';
    modalHtml += '<div class="modal-footer">';
    modalHtml += '<td><button id="addButton">Add More</button></td>';
    modalHtml += '<button type="button" class="btn btn-primary" data-dismiss="modal" id="btnaddsession" aria-label="Close">OK</button>';
    ...
    myHtml += '<table id="tbladdproduct" role="presentation" style="width:100%;border-collapse:collapse;border:0;border-spacing:0;background:#ffffff;">';
    myHtml += '<tr>';
    myHtml += '<th style="width: 150px;">Product</th>'; // Heading for Product
    myHtml += '<th style="width: 100px;">Quantity</th>'; // Heading for Quantity
    myHtml += '</tr>';
    myHtml += '<tr>';
    myHtml += '<td style="width: 150px;"><select class="select2 ddlProductmodel" id="ddlProductmodel" placeholder="Select Products" style="width: 80%;"></select></td>'; // Increase the z-index
    myHtml += '<td style="width: 100px;"><input type="text" id="textInput" class="form-control" placeholder="quantity"></td>';
    myHtml += '</tr>';
    myHtml += '</table>';
    myHtml += '</div>';
