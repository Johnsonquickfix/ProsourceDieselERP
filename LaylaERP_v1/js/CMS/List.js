...
function Status() {
    ...
    if (id == "") { swal('Alert', 'Please select pages from list', 'error'); }
        ...
        ActivityLog('Change pages status as ' + updatestatus + '', '/Cms/List');
        swal({ title: "", text: 'Would you like to ' + statusval + ' this page?', type: "question", showCancelButton: true })
            .then((result) => {
                if (result.value) {
                    $.ajax({
