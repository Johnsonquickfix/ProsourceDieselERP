    ...
    public class ProductRepository
    {
        ...
        public static DataTable Getcalculatemargins(string strValue1, string userstatus, string strValue3, string strValue4, string searchid, int pageno, int pagesize, out int totalrows, string SortCol = "order_id", string SortDir = "DESC")
        {
            ...
            try
            {
                ...
                string strSql = "Select TOP 1000 *, cast(sale_price as numeric(18,2)) - cast(cast_prise as numeric(18,2)) Margin,cast(regula_price as numeric(18,2)) - cast(cast_prise as numeric(18,2)) regulaMargin,coalesce(convert(numeric(18,2),( ((cast(sale_price as numeric(18,2)) - cast(cast_prise as numeric(18,2))) * 100) / NULLIF(cast(sale_price as numeric(18,2)),0))),0) marginpersantage,coalesce(convert(numeric(18,2),( ((cast(regula_price as numeric(18,2)) - cast(cast_prise as numeric(18,2))) * 100) / NULLIF(cast(regula_price as numeric(18,2)),0))),0) regularmarginpersantage "
              ...
              + " FROM wp_posts p "
              + " left join wp_postmeta as s on p.id = s.post_id and s.meta_key in ('_sku','_sale_price','_regular_price')"
              + " left join Product_Purchase_Items on Product_Purchase_Items.fk_product = p.id and is_active=1 and is_setprise = 1"
              + " WHERE p.post_type in ('product', 'product_variation') and p.post_status != 'draft' "
              + " GROUP BY  p.ID,guid,post_status,post_parent) tt"
               + " order by p_id,post_type,id";
