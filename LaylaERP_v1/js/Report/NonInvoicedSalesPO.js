...
function SendPO_Approval() {
    ...
    $.get("/Reports/GetNonInvoicedSalesPOList", option).then(response => {
        //  $("#loader").show();
...
function send_mail(id, result) {
    ...
    let inv_titleNew = 'Non-Invoiced Sales PO', po_authmail = data['po'][0].po_authmail;
    ...
    myHtml += '                <th style="width:10%;padding:5px 12px;text-align:right;font-family:sans-serif; font-size:15px; color:#4f4f4f;line-height:1.4;" class="itemquantity">SO No.</th>';
    ...
    $(data['pod']).each(function (index, tr) {
        ...
        myHtml += '    <td style="padding:5px 12px;text-align:left;font-family:sans-serif; font-size:15px; color:#4f4f4f;line-height:1.4;" class="itemdate_fk_projet">' + tr.fk_projet + '</td>';
    ...
    if (opt.strValue1.length > 1) {
        $.ajax({
            type: "POST", url: '/Reports/SendPOSalesMail', contentType: "application/json; charset=utf-8", dataType: "json", data: JSON.stringify(opt),
            beforeSend: function () {
                $("#loader").show();
            },
