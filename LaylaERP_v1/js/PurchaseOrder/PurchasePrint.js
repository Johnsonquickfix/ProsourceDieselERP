...
function printinvoice(id, result, is_mail, is_inv) {
    ...
    $(data['pod']).each(function (index, tr) {
        if (tr.product_type == 0) {
            ...
            else myHtml += '    <td style="padding:5px 12px;text-align:left;font-family:sans-serif; font-size:15px; color:#4f4f4f;line-height:1.4;" class="itemdescription">' + tr.description + '<br>Tag/Lot/Serial No. :- ' + (tr.fk_product > 0 ? '' : 'EX') + tr.product_serialno + '</td>';
