...
namespace LaylaERP.BAL
{
    public class PaymentInvoiceRepository
    {
        ...
        public static DataSet GetMiscPaymentType()
        {
            DataSet DS = new DataSet();
            try
            {
        ...
        public static DataTable GetMultipalPaymentMiscList(string flag, string payaccount, DateTime? fromdate, DateTime? todate, string searchid, int pageno, int pagesize, out int totalrows, string SortCol = "id", string SortDir = "DESC")
        {
            DataTable dt = new DataTable();
            totalrows = 0;
            try
            {
                ...
                DataSet ds = SQLHelper.ExecuteDataSet("erp_multipalpaymentmiscbill_search", parameters);
                dt = ds.Tables[0];
                if (ds.Tables[1].Rows.Count > 0)
                    totalrows = Convert.ToInt32(ds.Tables[1].Rows[0]["TotalRecord"].ToString());
            }
