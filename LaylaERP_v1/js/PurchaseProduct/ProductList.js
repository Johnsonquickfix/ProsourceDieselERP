...
function GetDetails() {
    var opt = { strValue1: '' };
    $.ajax({
        type: "POST", url: '/PurchaseProduct/GetCount', contentType: "application/json; charset=utf-8", dataType: "json", data: JSON.stringify(opt),
...
function dataGridLoad(order_type) {
    ...
    $('#dtdata').DataTable({
        ...
        sAjaxSource: "/PurchaseProduct/GetProductList",
...
function ChangeStatus(id, status) {
    ...
    ActivityLog('change status as ' + status + '', '/PurchaseProduct/UpdateproductcomponentStatus/' + id + '');
    var obj = { rowid: id, active: status, }
    $.ajax({
        url: '/PurchaseProduct/UpdateproductcomponentStatus/', dataType: 'json', type: 'Post',
...
function Status() {
    ...
    if (id == "") { swal('Alert', 'Please select product from list', 'error'); }
        ...
        ActivityLog('Change product status as ' + updatestatus + '', '/PurchaseProduct/ListProduct');
        ...
        swal({ title: "", text: 'Would you like to ' + statusval + ' this product?', type: "question", showCancelButton: true })
            .then((result) => {
                if (result.value) {
                    $.ajax({
                        url: '/PurchaseProduct/Changestatus', dataType: 'JSON', type: 'POST',
                        contentType: "application/json; charset=utf-8",
                        data: JSON.stringify(obj),
                        beforeSend: function () { $("#loader").show(); },
                        success: function (data) {
