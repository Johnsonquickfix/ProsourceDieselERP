...
function Search() { 
    var payment = $('#ddlPaymentType').val();
    if (payment == "0") { swal('Alert', 'Please select payment type', 'error'); }
    else {
...
function MiscBillGrid() {
    ...
    let payment = $('#ddlPaymentType').val();
    $('#dtdata').DataTable({
        ...
        sAjaxSource: "/PaymentInvoice/GetMultipalPaymentMiscList",
        fnServerData: function (sSource, aoData, fnCallback, oSettings) {
            aoData.push({ name: "strValue1", value: 'SERPB' }, { name: "strValue2", value: sd }, { name: "strValue3", value: ed }, { name: "strValue4", value: payment });
...
function MiscBillFullyGrid() {
    ...
    $('#dtfullypaid').DataTable({
        ...
        sAjaxSource: "/PaymentInvoice/GetMultipalPaymentMiscList",
        fnServerData: function (sSource, aoData, fnCallback, oSettings) {
            aoData.push({ name: "strValue1", value: 'SFRPB' }, { name: "strValue2", value: sd }, { name: "strValue3", value: ed }, { name: "strValue4", value: payment });
            if (oSettings.aaSorting.length > 0) { aoData.push({ name: "sSortColName", value: oSettings.aoColumns[oSettings.aaSorting[0][0]].data }); }
            oSettings.jqXHR = $.ajax({
                dataType: 'json', type: "GET", url: sSource, data: aoData,
                "success": function (data) {
...
function takepayment() {
    ...
    if (id == "") {
        ...
        var url = "/PaymentInvoice/MultipleBillPayment?status=" + status + "&id=" + id;
