...
namespace LaylaERP.Controllers
{
    public class PaymentInvoiceController : Controller
    {
        ...
        [HttpPost]
        public JsonResult TakePaymentPO(SearchModel model)
        {
            string JSONresult = string.Empty;
            try
            {
                    ...
                    foreach (var o in dyn)
                    {
                        ...
                        foreach (var det in dyndt)
                        {
                            ...
                            if (billno == bilnod)
                            {
                                ...
                                u_id = CommanUtilities.Provider.GetCurrent().UserID;
                                System.Xml.XmlDocument orderdetailsXML = JsonConvert.DeserializeXmlNode("{\"Data\":" + det + "}", "Items");
                                JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddAuthoriseMiscPayment(result, pkey, "POPD", u_id, result, orderdetailsXML));
                                break;
                            }
                            //resetLoop = true;
                ...
                {
                    result = clsPayPal.PaymentsPayouts(sender_batch_id, model.strValue4);
                    JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddNewMiscPayment(id, "POIC", u_id, result, model.strValue2));
                }
                else
                {
                    JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddNewMiscPayment(id, "POIC", u_id, result, model.strValue2));
        ...
        [HttpPost]
        public JsonResult TakePaymentSalesPO(SearchModel model)
        {
            string JSONresult = string.Empty;
            try
            {
                    ...
                    foreach (var o in dyn)
                    {
                        ...
                        foreach (var det in dyndt)
                        {
                            ...
                            if (billno == bilnod)
                            {
                                ...
                                u_id = CommanUtilities.Provider.GetCurrent().UserID;
                                System.Xml.XmlDocument orderdetailsXML = JsonConvert.DeserializeXmlNode("{\"Data\":" + det + "}", "Items");
                                JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddAuthoriseMiscPayment(result, pkey, "SOPD", u_id, result, orderdetailsXML));
                                break;
                            }
                            //resetLoop = true;
                ...
                {
                    result = clsPayPal.PaymentsPayouts(sender_batch_id, model.strValue4);
                    JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddNewMiscPayment(id, "SOIC", u_id, result, model.strValue2));
                }
                else
                {
                    JSONresult = JsonConvert.SerializeObject(PaymentInvoiceRepository.AddNewMiscPayment(id, "SOIC", u_id, result, model.strValue2));
